...
export function JobsContent({ initialJobs, initialSummary }: { initialJobs: Job[], initialSummary: JobSummary | null }) {
  ...
  useEffect(() => {
    // Skip initial load since we have initial data
    if (jobs === initialJobs && summary === initialSummary && !status) {
        return;
    }
    ...
    load();
    return () => { active = false; };
  }, [status, initialJobs, initialSummary, jobs, summary]);
      ...
      <div className="card">
        {loading ? (
          <div style={{ padding: "1.25rem" }}>
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="skeleton skeleton-row" style={{ height: "40px", marginBottom: "0.5rem" }} />
            ))}
          </div>
        ) : jobs.length === 0 ? (
